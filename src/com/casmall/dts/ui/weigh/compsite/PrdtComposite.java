...
public class PrdtComposite extends Composite{
	...
	private List<TsPrdtMstDTO> items;
	private List<TsPrdtMstDTO> filteredItems;
	 ...
	 * @param style
	 */
	public PrdtComposite(Composite parent, int style) {
		super(parent, style);
		initGUI();
	}
	...
	private void initGUI() {
		...
		tableCombo.defineColumns(new String[]{"관리코드","제품명","설명"});
		...
		tableCombo.addTextKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent e) {
				...
				if(txt.trim().length()==0){
					...
					tbl.removeAll();
					List<TsPrdtMstDTO> f = filter(txt.trim());
	...
	public static void showGUI() {
		...
		PrdtComposite inst = new PrdtComposite(shell, SWT.NULL);
	 ...
	 * @return
	 */
	private List<TableItem> loadThreeColumnDataset(List<TsPrdtMstDTO> modelList) {
		...
		for (int index=0; index < total; index++) {
			...
			TsPrdtMstDTO model = (TsPrdtMstDTO)modelList.get(index);
			ti.setText(new String[] { StringUtil.nullToBlank(model.getPrdt_mgt_cd()), model.getPrdt_nm(), 
				model.getNt()});
			rowList.add(ti);			
		}
	 ...
	 * @return
	 */
	public List<TsPrdtMstDTO> filter(String str){
		List<TsPrdtMstDTO> filtered = new ArrayList<TsPrdtMstDTO>();
		for(int i=0;i<items.size();i++){
			if(StringUtil.nullToBlank(((TsPrdtMstDTO)items.get(i)).getPrdt_mgt_cd()).indexOf(str)>-1 || ((TsPrdtMstDTO)items.get(i)).getPrdt_nm().indexOf(str)>-1){
	 ...
	 * @return
	 */
	public TsPrdtMstDTO getSelectionItem(){
		TsPrdtMstDTO model;
		int idx = tableComboViewer.getTableCombo().getSelectionIndex();
		if(idx >= 0){
			model = filteredItems.get(idx);
		}else{
			model = new TsPrdtMstDTO();
			model.setPrdt_nm(tableComboViewer.getTableCombo().getText());
		}
		return model;
	}
	 ...
	 * @param items
	 */
	public void setItems(List<TsPrdtMstDTO> items){
		this.items = items;
		this.filteredItems = items;
		loadThreeColumnDataset(items);
	}
	
	public List<TsPrdtMstDTO> getItems(){
		return items;
	}
	/**
    ...
    public String getValue(String key){
    	TsPrdtMstDTO dto = getSelectionItem();

    	if(KEY_ID.equals(key)){
    		return dto.getPrdt_cd();
    	}else if(KEY_NAME.equals(key)){
    		return dto.getPrdt_nm();
    	}else if(KEY_UP.equals(key)){
    		return ""+dto.getUnt_prc();
    	}else{
    		return "WRONG KEY";
    	}
    }
    ...
    public boolean setSelection(String data){
		...
		for(int i=0;i<filteredItems.size();i++){
			if(data.equals(filteredItems.get(i).getPrdt_cd())){
