 ...
 */
public class PrdtListComposite extends Composite implements CallbackIF{

	protected static Log log = LogFactory.getLog(PrdtListComposite.class);
	private Button btnSelect;
	private Composite ctTop;
	private Composite ctSelect;

	private GridTableViewer gridViewer;
	private String[] colProp = { "prdt_cd", "no", "prdt_mgt_cd", "prdt_nm", "mgt_yn", "unt_prc", "nt"};
	private String[] colName = { "", "No", "관리코드", "제품명", "관리", "단가", "비고"};
	private int[] colWidth = { 0, 8, 17, 25, 16, 12, 22};
	private int[] colAlign = { SWT.NONE, SWT.CENTER, SWT.CENTER, SWT.LEFT, SWT.CENTER, SWT.RIGHT, SWT.LEFT};
	private ArrayList<TsPrdtMstDTO> listData;
	...
	public static void showGUI() {
		...
		PrdtListComposite inst = new PrdtListComposite(shell, SWT.NULL);
	...
	public PrdtListComposite(org.eclipse.swt.widgets.Composite parent, int style) {
		super(parent, style);
...
//				gridViewer.getGrid().setRowHeaderVisible(true);
	...
	public class ListGridLabelProvider extends LabelProvider implements ITableLabelProvider, ITableFontProvider,
		...
		public String getColumnText(Object element, int columnIndex) {
			TsPrdtMstDTO data = (TsPrdtMstDTO) element;
	...
	private void selectData(){
		try{
			...
			listData = mgr.selectTsPrdtMst(dto);
	...
	private void btnExportMouseDown(MouseEvent evt){
	    ...
	    dialog.setFileName("제품정보 "+sdf.format(new Date())+".xls");
	    String saveFile = dialog.open();
	    if (saveFile == null) {
			return;
		}
	    saveExport(saveFile, "제품 정보");
	}
