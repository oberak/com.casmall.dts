 ...
 */
public class CarListComposite extends Composite implements CallbackIF{

	protected static Log log = LogFactory.getLog(CarListComposite.class);
	private Button btnSelect;
	private Composite ctTop;
	private Composite ctSelect;

	private GridTableViewer gridViewer;
	private String[] colProp = { "car_cd", "no", "car_mgt_cd", "car_num", "mgt_yn", "nt"};
	private String[] colName = { "", "No", "관리코드", "차량번호", "관리", "비고"};
	private int[] colWidth = { 0, 8, 17, 25, 14, 36};
	private int[] colAlign = { SWT.NONE, SWT.CENTER, SWT.CENTER, SWT.LEFT, SWT.CENTER, SWT.LEFT};
	private ArrayList<TsCarMstDTO> listData;
	...
	public static void showGUI() {
		...
		CarListComposite inst = new CarListComposite(shell, SWT.NULL);
	...
	public CarListComposite(org.eclipse.swt.widgets.Composite parent, int style) {
		super(parent, style);
...
//				gridViewer.getGrid().setRowHeaderVisible(true);
	...
	public class ListGridLabelProvider extends LabelProvider implements ITableLabelProvider, ITableFontProvider,
		...
		public String getColumnText(Object element, int columnIndex) {
			TsCarMstDTO data = (TsCarMstDTO) element;
	...
	private void selectData(){
		try{
			...
			listData = mgr.selectTsCarMst(dto);
	...
	private void btnExportMouseDown(MouseEvent evt){
	    ...
	    dialog.setFileName("차량정보 "+sdf.format(new Date())+".xls");
	    String saveFile = dialog.open();
	    if (saveFile == null) {
			return;
		}
	    saveExport(saveFile, "차량 정보");
	}
