		...
		for(TsPrtAttrDTO dto : atrList){
			...
			if(DTSConstants.CD_ATTR_FLAG_DB.equals(dto.getAttr_flg_cd())){
				...
				if(preferences.getBoolean(DTSPreConstants.DATA_PRDT_FLAG))
					prtStr += "\n상품명:"+data.getPrdt_nm();
				prtStr += "\n1차계량일시:"+StringUtil.getString(data.getFst_wgt_dt(), "yyyy-MM-dd HH:mm");
				prtStr += "\n1차계량:"+StringUtil.getString(data.getFst_wgh(),"#,##0");
				prtStr += "\n2차계량일시:"+StringUtil.getString(data.getScnd_wgt_dt(), "yyyy-MM-dd HH:mm");
				prtStr += "\n2차계량:"+StringUtil.getString(data.getScnd_wgh(),"#,##0");
				if(preferences.getBoolean(DTSPreConstants.DATA_PRICE_FLAG)){
					prtStr += "\n단가:"+StringUtil.getString(data.getUnt_prc(),"#,##0");
					prtStr += "\n금액:"+StringUtil.getString(data.getAmt(),"#,##0");
				}
